...
def landing_view(request):
    ...
    # Load any manual year counts from system settings
    ...
    last_year_completed = five_year_stats[0]['count'] if five_year_stats else 0
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            login(request, user)
            redirect_url = reverse('core:change_initial_password') if user.must_change_password else reverse('core:dashboard')
            if request.headers.get('x-requested-with') == 'XMLHttpRequest':
...
def change_initial_password_view(request):
    ...
    if request.method == 'POST':
        ...
        if password != confirm_password:
            return render(request, 'core/change_initial_password.html', {
...
def register_view(request):
    ...
    if not system_settings.registration_enabled:
        ...
        params = urlencode({
            'school': school_ctx['school_code'],
            'registration_closed': '1',
            'message': message,
        })
        return redirect(f"{reverse('core:login')}?{params}")
    if request.method == 'POST':
...
def _ensure_period_assignments(period):
    ...
    if not period.lecturer_id and period.student.course and company_name:
        lecturers = User.objects.filter(
            role='LECTURER',
            course=period.student.course,
        ).order_by('id')
        for lecturer in lecturers:
            if _companies_match(company_name, lecturer.institution_or_company):
    ...
    if user.role == 'LECTURER':
        ...
        if password:
            message += '\n'.join([
                ...
                '',
                'Use this temporary password to sign in. You will be required to create a new password immediately after signing in.',
            ])
        else:
            message += '\n'.join([
                '',
                'Set a new password using this secure link before signing in:',
                password_reset_confirm_url,
            ])
        message += '\n'.join([
            '',
...
def dashboard_view(request):
    ...
    if user.must_change_password:
        return redirect('core:change_initial_password')
    ...
    if user.role == 'STUDENT':
        ...
        if request.method == 'POST' and request.POST.get('organization_update_form') == '1':
            organization = request.POST.get('institution_or_company', '').strip()
            if not organization:
                messages.error(request, 'Please enter the organization where you are undertaking your attachment.')
            elif len(organization) > 255:
                messages.error(request, 'The organization name must be 255 characters or fewer.')
            else:
                previous_organization = user.institution_or_company
                user.institution_or_company = organization
                user.save(update_fields=['institution_or_company'])

                assignment_fields = []
                if period.supervisor_id and not _companies_match(
                    organization, period.supervisor.institution_or_company
                ):
                    period.supervisor = None
                    assignment_fields.append('supervisor')
                if period.lecturer_id and (
                    not _companies_match(organization, period.lecturer.institution_or_company)
                    or period.lecturer.course != user.course
                ):
                    period.lecturer = None
                    assignment_fields.append('lecturer')
                if assignment_fields:
                    period.save(update_fields=assignment_fields)

                # Keep a draft supervisor request in sync when it used the
                # previous organization.
                if not period.supervisor_id and period.field_supervisor_organization == previous_organization:
                    period.field_supervisor_organization = organization
                    period.save(update_fields=['field_supervisor_organization'])

                _ensure_period_assignments(period)
                messages.success(request, 'Your attachment organization and matching assignments have been updated.')
            return redirect('core:dashboard')
        ...
        # Archived/approved reports (not pending review or returned)
        archived_reports = AttachmentPeriod.objects.filter(
            final_report__isnull=False
        ).exclude(final_report='').exclude(report_status__in=['PENDING_REVIEW', 'RETURNED']).select_related('student').order_by('-id')
...
def admin_dashboard_view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'save_system_settings':
            if request.user.role not in ['ADMIN', 'ATTACHMENT_ADMIN']:
                return HttpResponseForbidden("Only administrators can manage portal settings.")
            system_settings.term_start_date = request.POST.get('term_start_date') or None
            system_settings.term_end_date = request.POST.get('term_end_date') or None
            system_settings.submission_deadline_date = request.POST.get('submission_deadline_date') or None
            system_settings.logbook_lock_date = request.POST.get('logbook_lock_date') or None
            system_settings.default_new_account_role = request.POST.get('default_new_account_role', 'STUDENT')
            system_settings.require_password_reset = request.POST.get('require_password_reset') == 'Yes'
            system_settings.placement_approval = request.POST.get('placement_approval', 'MANUAL')
            system_settings.notification_frequency = request.POST.get('notification_frequency', 'WEEKLY')
            system_settings.audit_retention = request.POST.get('audit_retention', '90 days')
    ...
    for i in range(5):
        ...
        count = yr_qs.count()
        # If a manual override exists for this academic year, use it
        manual_val = None
        if yr_label in manual_counts:
            try:
                manual_val = int(manual_counts.get(yr_label))
                count = manual_val
            except Exception:
                manual_val = None
        total_5yr_completed += count
        five_year_stats.append({'year': yr_label, 'count': count, 'manual_value': manual_val})
    ...
    # Find pending supervisor requests (student-submitted) with no assigned supervisor yet
    try:
        pending_qs = AttachmentPeriod.objects.filter(supervisor__isnull=True).filter(
            Q(field_supervisor_email__isnull=False) | Q(field_supervisor_name__isnull=False)
        ).order_by('-id')
        if school_filter:
            pending_qs = pending_qs.filter(student__school=school_filter)
        pending_requests = list(pending_qs)
        pending_count = pending_qs.count()
    except Exception:
        pending_requests = []
        pending_count = 0
    # Pending weekly logs needing supervisor approval (for Reports -> Pending Reviews)
    try:
        pending_reviews_qs = WeeklyLog.objects.filter(supervisor_approved=False)
        if school_filter:
            pending_reviews_qs = pending_reviews_qs.filter(profile__student__school=school_filter)
        pending_reviews_list = list(pending_reviews_qs.order_by('-id')[:100])
    except Exception:
        pending_reviews_list = []
    # Always attempt to fetch unread admin notifications for the current admin user
    admin_notifications = []
...
@login_required
@require_POST
def admin_update_historic_counts_view(request):
    if not User.is_admin_console_user(request.user.role):
        return HttpResponseForbidden("Only administrators can update historic counts.")

    # Expecting POST data `manual_counts[2025/2026]=4` etc.
    manual = {}
    for key, val in request.POST.items():
        if key.startswith('manual_counts[') and key.endswith(']'):
            year = key[len('manual_counts['):-1]
            try:
                manual[year] = int(val)
            except Exception:
                manual[year] = val
    ...
    system_settings = SystemSettings.get_settings()
    system_settings.landing_manual_year_counts = manual
    # Update summary fields for convenience
    try:
        last_year = sorted(manual.keys(), reverse=True)[0] if manual else None
        system_settings.landing_last_year_completed = int(manual.get(last_year, system_settings.landing_last_year_completed)) if last_year else system_settings.landing_last_year_completed
        system_settings.landing_total_last_5_years = sum(int(v) for v in manual.values()) if manual else system_settings.landing_total_last_5_years
    except Exception:
        pass
    system_settings.save()

    messages.success(request, 'Historic attachment counts updated.')
    return redirect('core:admin_dashboard')
...
def admin_create_user_view(request):
    ...
    if selected_role == 'LECTURER' and not post_data.get('password', '').strip():
        generated_password = _generate_temporary_password()
        post_data['password'] = generated_password
        post_data['confirm_password'] = generated_password
    ...
    try:
        if form.is_valid():
            with transaction.atomic():
                ...
                created_user.must_change_password = selected_role == 'LECTURER'
                created_user.save()
                if selected_role == 'LECTURER':
            ...
            try:
                _send_new_account_email(
                    request,
                    created_user,
                    form.cleaned_data.get('password') or generated_password,
                )
        ...
        template_name = 'core/attachment_admin_dashboard.html' if request.user.role == 'ATTACHMENT_ADMIN' else 'core:admin_dashboard'
        # Provide minimal admin-dashboard context to avoid template lookup errors
        system_settings = SystemSettings.get_settings()
        current_academic_year = getattr(system_settings, 'current_academic_year', '2025/2026') or '2025/2026'
...
def admin_export_attachment_data_view(request):
    ...
    # Download all attachment records (both active and archived) unless a specific filter is provided
    periods_qs = AttachmentPeriod.objects.select_related('student', 'supervisor', 'lecturer')
